  ...
  return (
    <div className="p-6 max-w-5xl mx-auto mt-8">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Escanea y comprueba el flujo de los códigos (objetos, pistas, llaves) antes de imprimirlos.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="border rounded-xl p-6 bg-white shadow-sm">
          ...
          {!validationResult && !scanMode && (
            <div className="h-full flex items-center justify-center pb-8">
              <p className="text-gray-400 italic">Esperando lectura de código QR...</p>
            </div>
          )}

          {scanMode && !validationResult && (
            <div className="h-full flex items-center justify-center pb-8">
              <p className="text-blue-500 italic">Analizando payload del código...</p>
            </div>
          )}
            ...
            <div className="space-y-4 animate-[fadeIn_0.3s_ease-in-out]">
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg text-green-800">
                <p className="font-bold text-xl mb-1 flex items-center gap-2">
                  <span>✅</span> Código Válido y Seguro
                </p>
                <p className="text-sm text-green-600">La estructura del QR es compatible con SAGA v0.5.4</p>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 p-3 rounded border">
                  <p className="text-xs text-gray-500 uppercase font-bold">Tipo de Entidad</p>
                  <p className="font-semibold text-gray-800 capitalize">{validationResult.type}</p>
                </div>
                <div className="bg-gray-50 p-3 rounded border">
                  <p className="text-xs text-gray-500 uppercase font-bold">Identificador</p>
                  <p className="font-semibold text-gray-800">{validationResult.name}</p>
                </div>
              </div>

              <div className="bg-gray-50 p-4 rounded border">
                <p className="text-sm text-gray-700 mb-2 font-bold">Simulación de Reglas:</p>
                <ul className="space-y-2 text-sm">
                  <li className="flex items-center gap-2">
                    ...
                    Requisitos de desbloqueo alcanzables
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="w-5 h-5 bg-green-100 text-green-700 rounded-full flex items-center justify-center text-xs">✓</span>
                    Conflictos de inventario: {validationResult.conflicts}
                  </li>
                </ul>
              </div>
            </div>
